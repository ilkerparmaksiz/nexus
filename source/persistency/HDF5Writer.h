...
namespace nexus {
  ...
  public:
    ...
    void WriteStep(int64_t evt_number,
                   ...
                   float time);
    void WriteOpticksHitInfo(hit_opticks_t *ohit);
    void WriteOpticksStep(int64_t evt_number,int32_t indx, const char *flag,unsigned int boundary, unsigned  int identiy,std::vector<float> vals);
  ...
  private:
    ...
    size_t oistep_; ///< counter for opticks steps
